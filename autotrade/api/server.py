...
from autotrade.broker.alpaca_client import broker
...
async def prices(syms: str = "NVDA,AAPL,MSFT,TSLA"):
    symbols: List[str] = [s.strip() for s in syms.split(",") if s]
    return await asyncio.to_thread(broker.get_prices, symbols)
